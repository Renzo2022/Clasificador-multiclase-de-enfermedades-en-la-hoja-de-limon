...
const EnhanceDiseaseInformationInputSchema = z.object({
  diseaseName: z.string().describe('The name of the predicted disease or condition (e.g., "spider_mites", "Healthy", "Undetermined").'),
  confidence: z.number().optional().describe('The confidence score of the prediction (0.0 to 1.0). Only relevant if diseaseName is not "Undetermined".'),
});
...
const EnhanceDiseaseInformationOutputSchema = z.object({
  comprehensiveGuide: z.string().describe('A comprehensive guide including disease details, remedies, and prevention advice, formatted for display. For "Undetermined", this includes image advice and general care.'),
});
...
If the "Predicted Condition" is "Undetermined":
Provide a message and "General Lemon Tree Care & Observation" advice.
The message should state:
"### Unable to Determine Condition
The AI could not confidently determine the specific condition of the lemon leaf from the provided image. This can happen if the image is unclear, out of focus, taken from too far away, or doesn't clearly show distinctive symptoms of a common lemon leaf disease.

**For a more accurate diagnosis, please try the following:**
*   Use a clear, high-resolution image.
*   Ensure the lemon leaf is well-lit and in focus.
*   Capture the affected areas of the leaf closely.
*   Make sure the leaf is the main subject of the photo.

"
Following this message, provide "### General Lemon Tree Care & Observation" advice. This should include key symptoms of common problems to watch for, general care best practices (watering, sunlight, fertilization), and when to consider seeking professional advice. Use Markdown for headings and lists. Do not mention a confidence score for the "Undetermined" state.

For all other disease conditions (not "Healthy" or "Undetermined"):
...
const enhanceDiseaseInformationFlow = ai.defineFlow(
  ...
  async input => {
    const promptInput = {
      ...input,
      // Round confidence for display, but only if diseaseName is not "Undetermined"
      confidence: (input.diseaseName !== "Undetermined" && input.confidence) ? Math.round(input.confidence * 100) : undefined,
    };
